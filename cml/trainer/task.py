...
KEY_FEATURE_COLUMN = 'time'
TARGET_FEATURE_COLUMN = 'value'
...
def get_vocabulary(metadata_path):
  """Returns the CML species vocabulary in the order of its numeric index.
...
def get_export_signature_fn(metadata_path):
  ...
  def get_export_signature(examples, features, predictions):
    ...
    cml_labels = get_vocabulary(metadata_path)
    prediction = tf.argmax(predictions, 1)
    labels = tf.contrib.lookup.index_to_string(
        prediction, mapping=cml_labels, default_value=UNKNOWN_LABEL)
...
def main(argv=None):
  """Run a Tensorflow model on the cml dataset."""
