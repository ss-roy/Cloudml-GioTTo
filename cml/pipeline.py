"""cml Classification Sample Cloud Runner.
"""
import argparse
import datetime
import os
...
import model as cml
#import trainer.task as task #ImportError: No module named trainer.task
#from ..trainer.task #SYNTAX ERROR
#import task as task #from .import util ValueError: Attempted relative import innonpackage
#import task #import util error
...
def parse_arguments(argv):
  parser = argparse.ArgumentParser(
      description='Runs Training on the cml model data.')
  parser.add_argument(
...
class LazyVocabLoader(object):
  ...
  def get_vocab(self):
    # Returns a dictionary of cml labels to consecutive integer identifiers.
    if not self.vocab:
      yaml_data = ml.features.FeatureMetadata.get_metadata(self.metadata_path)
      self.vocab = yaml_data.columns['value']['vocab']
    return self.vocab

  def get_reverse_vocab(self):
    # Returns a list of consecutive integer identifiers to cml labels.
    if not self.reverse_vocab:
      vocab = self.get_vocab()
      self.reverse_vocab = [None] * len(vocab)
      for value_name, index_number in vocab.iteritems():
